...
class Lyricist extends Model implements HasMedia
{
    ...
    {
        return route('frontend.lyricist', ['id' => $this->attributes['id'], 'slug' => str_slug($this->attributes['name']) ? str_slug(html_entity_decode($this->attributes['name'])) : str_replace(' ', '-', html_entity_decode($this->attributes['name']))]);
    }

    public function getSongCountAttribute($value)
    {
        if ($this->id < 1000) {
            return Song::where('lyricistIds', 'REGEXP', '(^|,)(' . $this->id . ')(,|$)')->count();
        } else {
            return Song::whereRaw("lyricistIds LIKE CONCAT('". $this->id . "', '%')")->count();
        }
    }
    ...
    {
        if ($this->id < 1000) {
            return Album::where('lyricistIds', 'REGEXP', '(^|,)(' . $this->id . ')(,|$)')->count();
        } else {
            return Album::whereRaw("lyricistIds LIKE CONCAT('". $this->id . "', '%')")->count();
        }
    }
    ...
    {
        if ($this->id < 1000) {
            return Song::where('lyricistIds', 'REGEXP', '(^|,)(' . $this->id . ')(,|$)');
        } else {
            return Song::whereRaw("lyricistIds LIKE CONCAT('". $this->id . "', '%')");
        }
    }
    ...
    {
        if ($this->id < 1000) {
            return Album::where('lyricistIds', 'REGEXP', '(^|,)(' . $this->id . ')(,|$)');
        } else {
            return Album::whereRaw("lyricistIds LIKE CONCAT('". $this->id . "', '%')");
        }
    }

    public function similar()
    {
        return Lyricist::whereIn('genre', explode(',', $this->genre));
    }
