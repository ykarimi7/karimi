...
@section('content')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ route('backend.artists.edit', ['id' => $artist->id]) }}">{!! $artist->name !!}</a></li>
        <li class="breadcrumb-item active">Upload</li>
    </ol>
    <div class="row">
        <div class="col-12">
            <div class="uploaded-files card-columns card-2-columns"></div>
            <form id="fileupload" data-template="template-upload" method="POST" enctype="multipart/form-data" action="{{ route('backend.artist.upload.bulk', ['artistId' => $artist->id]) }}">
