...
@section('content')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Lyricists & Songwriters ({{ $lyricists->total() }})</li>
    </ol>
    <div class="row">
        <div class="col-lg-12">
            <div class="card shadow mb-4">
                <div class="card-header py-3 border-0">
                    <button class="btn btn-link p-0 m-0" type="button" data-toggle="collapse" data-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
                        <h6 class="m-0 font-weight-bold text-primary">Advanced lyricist search</h6>
                    </button>
                    <a href="{{ route('backend.lyricists.add') }}" class="btn btn-primary btn-sm float-right">Add new lyricist</a>
                </div>
                <div class="card-body p-0">
                    <div class="accordion" id="collapseMetaTags">
                        <div id="collapseOne" class="collapse p-4" aria-labelledby="headingOne" data-parent="#collapseMetaTags">
                            <form class="search-form" action="{{ route('backend.lyricists') }}">
                                <div class="form-group row">
                                    <label class="col-sm-2 col-form-label">Keyword</label>
                                    <div class="col-sm-10">
                                        <input type="text" name="term" class="form-control" placeholder="Lyricist nane..." value="{{ request()->input('term') }}">
                                    </div>
                                </div>
                                <div class="form-group row">
                                ...
                                <a href="{{ route('backend.lyricists') }}" class="btn btn-secondary">Clear</a>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
            <form id="mass-action-form" method="post" action="{{ route('backend.lyricists.mass.action') }}">
                @csrf
                <table class="table table-striped datatables table-hover">
                    ...
                    <tbody>
                    @include('backend.commons.lyricist', ['lyricists' => $lyricists])
                    </tbody>
                </table>
                <div class="row">
                    <div class="col-6">{{ $lyricists->appends(request()->input())->links() }}</div>
                    <div class="col-6">
