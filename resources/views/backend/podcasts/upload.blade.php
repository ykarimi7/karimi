...
@section('content')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ route('backend.podcasts') }}">{{ $podcast->title }}</a></li>
        <li class="breadcrumb-item active">Upload</li>
    </ol>
    <div class="row">
        <div class="col-12">
            <div class="uploaded-files card-columns card-2-columns"></div>
            <form id="fileupload" method="POST" enctype="multipart/form-data" action="{{ route('backend.podcast.upload.bulk', ['podcast_id' => $podcast->id]) }}">
    ...
    @include('backend.commons.upload-episode-item')
@endsection