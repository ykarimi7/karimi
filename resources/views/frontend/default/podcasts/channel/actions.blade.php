@if(! auth()->check() || auth()->check() && auth()->user()->id != $channel->user->id)
    <a class="btn btn-favorite favorite @if($channel->favorite) on @endif" data-type="playlist" data-id="{{ $channel->id }}" data-title="{{ $channel->title }}" data-url="{{ $channel->permalink_url }}" data-text-on="{{ __('web.PLAYLIST_UNSUBSCRIBE') }}" data-text-off="{{ __('web.PLAYLIST_SUBSCRIBE') }}">
...
@if($channel->visibility)
    <a class="btn share desktop" data-type="playlist" data-id="{{ $channel->id }}">
...
@if (auth()->check() && auth()->user()->id == $channel->user->id)
    <a class="btn dropdown desktop edit-playlist-context-trigger" data-type="playlist" data-id="{{ $channel->id }}">
...
@if (auth()->check() && auth()->user()->id == $channel->user->id)
    <a class="btn edit-playlist-context-trigger mobile" data-type="playlist" data-id="{{ $channel->id }}">
...
@endif
<a class="btn play play-object mobile" data-type="playlist" data-id="{{ $channel->id }}">
    <span data-translate-text="PLAY_SONG">Play</span>
</a>
<a class="btn options mobile" data-toggle="contextmenu" data-trigger="left" data-type="playlist" data-id="{{ $channel->id }}">
